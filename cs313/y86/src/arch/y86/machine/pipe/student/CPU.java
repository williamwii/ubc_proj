...
public class CPU extends AbstractY86CPU.Pipelined {
  ...
  @Override protected void pipelineHazardControl () throws Register.TimingException {
      // Load - Use hazard
      int srcA = d.srcA.getValueProduced();
      int srcB = d.srcB.getValueProduced();
      int mDstM = e.dstM.getValueProduced();
      if ((srcA != R_NONE && mDstM == srcA) || (srcB != R_NONE && mDstM == srcB)) {
	  F.stall  = true;
	  D.stall  = true;
	  E.bubble = true;
      }
      ...
      // Control Hazard: JMP
      // Shoot down execute and decode if prediction is wrong
      else if (e.iCd.getValueProduced() == I_JXX && e.iFn.getValueProduced() != C_NC) {
	  // Backward jump is taken while it should not be
	  if (e.cnd.getValueProduced() == 0 && e.valP.getValueProduced() > e.valC.getValueProduced()) {
	      	D.bubble = true;
	  	E.bubble = true;
	  }
	  // Forward jump is not taken while it should be
	  else if (e.cnd.getValueProduced() == 1 && e.valP.getValueProduced() <= e.valC.getValueProduced()) {
	      D.bubble = true;
	      E.bubble = true;
	  }
      }
  ...
  @Override protected void fetch_SelectPC () throws Register.TimingException {
      // Not-taken backward jump in M
      if (M.iCd.get()==I_JXX && M.iFn.get()!=C_NC && M.cnd.get() == 0 && M.valP.get() > M.valC.get()) 
	f.pc.set (M.valP.get());
      
      // Should-be-taken forward jump in M
      else if (M.iCd.get()==I_JXX && M.iFn.get()!=C_NC && M.cnd.get() == 1 && M.valP.get() <= M.valC.get())
	f.pc.set(M.valC.get());
  ...
  private void fetch_PredictPC () throws Register.TimingException {
	...
	switch (f.iCd.getValueProduced()) {
		case I_JXX:
		    // Predict backward jumps are taken
		    if (f.pc.getValueProduced() > f.valC.getValueProduced()) {
  ...
  private int decode_ReadRegisterWithForwarding (int regNum) throws RegisterSet.InvalidRegisterNumberException, Register.TimingException
  {
	  ...
	  // Forward valE from execute
          if (e.dstE.getValueProduced() == regNum && e.cnd.getValueProduced() == 1) {
    	  	return e.valE.getValueProduced();
          }
          // Forward valE from memory
          else if (m.dstE.getValueProduced() == regNum && m.cnd.getValueProduced() == 1) {
    	  	return m.valE.getValueProduced();
          }
          // Forward valM from memory
          else if (m.dstM.getValueProduced() == regNum && m.cnd.getValueProduced() == 1) {
    	  	return m.valM.getValueProduced();
          }
          // Forward valE from write back
          else if (W.dstE.get() == regNum && W.cnd.get() == 1) {
              return W.valE.get();
          }
          // Forward valM from write back
          else if (W.dstM.get() == regNum && W.cnd.get() == 1) {
              return W.valM.get();
          }
          // No forwarding
          else {
