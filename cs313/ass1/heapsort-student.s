...
###
	pushl %esi				# store previous %esi value
	rrmovl %eax, %esi		# initialize i = last
	call heapify_array		# heapify_array(last)
	
heapsort_loop:
	rrmovl %esi, %eax		# %eax = last, extract_max needs last as argument
	call extract_max
	irmovl $4, %ebx			##
	rrmovl %esi, %ecx		## calculate the correct memory location
	mull %ebx, %ecx			##
	rmmovl %eax, heap(%ecx) # store the max in last location
	irmovl $1, %edx
	subl %edx, %esi			# last--
	jge heapsort_loop		# continue the loop if last >= 0
