 ...
 *
 */
public class CongaSetup implements IBeastProvider {
	/**
	 * My sweet, beasts.
	 */
	 ...
	 */
	public CongaSetup(int width, int height) {
		beasties = new Beast[6];
		beasties[0] = new Beast(new SpinnyBrain(), new CongaPackFinder(this));
		beasties[1] = new Beast(new FollowerBrain(), new CongaPackFinder(this));
		beasties[2] = new Beast(new LonelyBrain(), new CongaPackFinder(this));
		beasties[3] = new Beast(new RandomWalkingBrain(), new CongaPackFinder(this));
		beasties[4] = new Beast( new GroupieBrain(), new CongaPackFinder(this) );
		beasties[5] = new Beast( new FlockBrain(), new CongaPackFinder(this) );

		for (int i=0; i <beasties.length;i++){
			grooveBeastRandomly(beasties[i]);
