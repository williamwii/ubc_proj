...
public class PhotoManagerTest {
	...
	@Test
	public void testGetPhotosInDateRange() throws PhotoAlreadyInAlbumException {
		aPhotoManager.addAlbum(album1);
		aPhotoManager.addAlbum(album2);
	...
	@Test
	public void testNoPhotosInDateRange() throws PhotoAlreadyInAlbumException {
		aPhotoManager.addAlbum(album1);
		aPhotoManager.addAlbum(album2);
		photo1.setDateAdded(new Date(100000));
		photo2.setDateAdded(new Date(200000));
	...
	@Test
	public void testMovingPhotos()throws PhotoAlreadyInAlbumException, PhotoDoesNotExistException{
		aPhotoManager.addAlbum(album1);
		aPhotoManager.addAlbum(album1a);
		aPhotoManager.addAlbum(album2);
		assertEquals(2,aPhotoManager.getAlbums().size());
		assertEquals(3,aPhotoManager.getPhotos().size());
		assertTrue(aPhotoManager.getPhotos().contains(photo1));
		assertTrue(aPhotoManager.getPhotos().contains(photo2));
		assertTrue(aPhotoManager.getPhotos().contains(photo3));
		
		aPhotoManager.removePhoto(photo1);
		assertEquals(album1.getPhotos().size(), 1);
		assertEquals(aPhotoManager.getPhotos().size(), 2);
		
		aPhotoManager.addPhoto(photo4, album1);
		assertTrue(album1.getPhotos().contains(photo4));
		assertTrue(aPhotoManager.getPhotos().contains(photo4));
		
		aPhotoManager.movePhoto(photo2, album2);
		assertFalse(album1.getPhotos().contains(photo2));
