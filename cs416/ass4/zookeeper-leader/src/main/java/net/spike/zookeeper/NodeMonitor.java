...
public class NodeMonitor implements Watcher, AsyncCallback.ChildrenCallback {
    ...
    public void processResult(int rc, String path, Object o, List<String> children) {
        ...
        if(rc == KeeperException.Code.Ok) {
            if(getLowestNumber(children) == processNumber)
    ...
    public long getLowestNumber(List<String> children) {
        return getLargestProcessNumber(children);
    ...
    public long getLargestProcessNumber(List<String> children) {
        long largest = processNumber;
        for (String child : children) {
            long current = parseProcessNumber(child);
    ...
    public long parseProcessNumber(String znode) {
        return Integer.parseInt(znode.substring(znode.lastIndexOf("-") + 1, znode.lastIndexOf(".")));
    }
    ...
    public long createZnode() throws InterruptedException, KeeperException {
        long processNumber = ((int)Math.floor(Math.random() * 999)) + 1;
        String znode = this.znode.substring(0, this.znode.lastIndexOf("-") + 1) + String.format("%03d", processNumber) + this.znode.substring(this.znode.lastIndexOf("."));
        this.znode = zooKeeper.create(znode, new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);

        logger.debug("Created znode: " + this.znode);
        return parseProcessNumber(this.znode);
    }
