...
{   
    ...
    char* vlan_input = (char*) (malloc(256 * sizeof(char)));
    
    // Extra \n is found, fgets one more time to remove that.
	fgets(vlan_input,256,file);
	while (strcmp(vlan_input,"\n")==0)
		fgets(vlan_input,256,file);
    sp = strtok_r(vlan_input, " \n", &ep);
    int x;
    while (sp!=NULL){
        portTable[x].v_lan = sp;
        sp = strtok_r(ep, " \n", &ep);
        x++;
    }
    ...
    while (fgets(input, 35, file)!=NULL){
        if (strcmp(input,"\n")!=0){
            ...
            //fprintf(stdout, "%s\n%s\n%s\n%s\n\n",p_name,MAC_Source_Addr,MAC_Dest_Addr,id);
            ...
            // not found in all the ports, broadcast the packet to the ones
            // in the same group.
            // If dest port is same with source port, drop the packet.
            // Else, send to destination port if destination port is in
            // the same group with source port.
            ...
            if ((d_addr==BROADCAST_ADDR)||(dest_index==-1)){
                for (i=0;i<port_num;i++){
                    if ((i!=input_index)&&(strcmp(portTable[i].v_lan, portTable[input_index].v_lan)==0))
                        portTable[i].counter++;
                }
                dest = "BROADCAST";
                vlan = portTable[input_index].v_lan;
            }
            else if ((input_index==dest_index)||(strcmp(portTable[input_index].v_lan, portTable[dest_index].v_lan)!=0))
                dest = "DROP";
            else{
    ...
    int i;
    for (i=0;i<port_num;i++){
        fprintf(stdout, "%s %d %s\n",portTable[i].portname,portTable[i].counter,portTable[i].v_lan);
    }
    ...
    for (k=0;k<total_mac;k++){
        fprintf(stdout, "%012llx %s %s\n",all_mac[k]->addr,all_mac[k]->port,all_mac[k]->vlan);
    }
